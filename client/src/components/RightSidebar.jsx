...
const RightSidebar = ({ selectedUser }) => {
  return selectedUser && (
    <div className={`bg-[#8185B2]/10 text-white w-full relative overflow-y-scroll`}>
      <div className={`pt-16 flex flex-col items-center w-full relative overflow-y-scroll ${selectedUser ? "max-md:hidden" : ""}`}>
      ...
      <div className='px-5 text-xs'>
        ...
        <div className='mt-2 max-h-50 overflow-y-scroll grid grid-cols-2 gap-4 opacity-80'>
          {imagesDummyData.map((url, index) => (
            <div key={index} onClick={()=> window.open(url)} className='cursor-pointer rounded-2xl'>
